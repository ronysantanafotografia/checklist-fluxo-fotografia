...
const STORAGE_KEY = "eks_checklist_jobs_v7";
const BRAND_KEY = "eks_studioflow_brand_v1";
...
const TASK_TEMPLATES = {
  [`${PROJECT_TYPE.ENSAIO}__${DELIVERY_MODE.DIGITAL}`]: [
    ...
    { id: "client_chose_images", title: "CLIENTE SELECIONOU AS IMAGENS NO DIA", hasDate: true, extraChoice: true },
    { id: "final_treatment_export", title: "TRATAMENTO FINAL DAS IMAGENS E EXPORTAR (INTERNET, TRATADAS)" },
    { id: "send_images_client", title: "ENVIAR IMAGENS PARA O CLIENTE (Em baixa no WhatsApp e em alta no MyAirBridge)" },
    { id: "delivery_prep", title: "PREPAROU A ENTREGA (Gravar imagens em alta no pen drive, imprimir 3 fotos, cartão de agradecimento e feedback, mimos, etc.)" },
  ...
  [`${PROJECT_TYPE.EVENTO}__${DELIVERY_MODE.DIGITAL}`]: [
    ...
    { id: "delivery_prep", title: "PREPAROU A ENTREGA (Gravar imagens em alta no pen drive, imprimir 5 fotos, cartão de agradecimento e feedback, mimos, etc.)" },
    { id: "delivered", title: "ENTREGOU / INFORMOU PARA O CLIENTE QUE FOI FINALIZADO NO DIA", hasDate: true },
  ],
  ...
  [`${PROJECT_TYPE.ENSAIO}__${DELIVERY_MODE.DIGITAL_ALBUM}`]: [
    ...
    { id: "delivery_prep", title: "PREPAROU A ENTREGA (Gravar imagens em alta no pen drive, imprimir cartão de agradecimento e feedback, mimos, etc.)" },
    { id: "delivered", title: "ENTREGOU / INFORMOU PARA O CLIENTE QUE FOI FINALIZADO NO DIA", hasDate: true },
  ],
...
function loadBrand() {
  try {
    ...
    if (!raw) return { name: "", logoDataUrl: "" };
    const data = JSON.parse(raw);
    return {
...
export default function Home() {
  ...
  // ✅ Branding
  const [brand, setBrand] = useState({ name: "", logoDataUrl: "" });
  ...
  useEffect(() => {
    ...
    if (view === "PENDENTES" && active.isCompleted) setActiveId(pendingJobs?.[0]?.id || null);
    if (view === "FINALIZADOS" && !active.isCompleted) setActiveId(doneJobs?.[0]?.id || null);
  ...
  function reopenProject() {
    if (!active) return;
    setJobs((prev) =>
      prev.map((j) => (j.id === active.id ? { ...j, isCompleted: false, completedAt: "" } : j))
    );
  ...
  function onPickLogo(file) {
    ...
    // limita para imagens pequenas (evita estourar localStorage)
    const maxBytes = 700 * 1024;
    if (file.size > maxBytes) {
      alert("A imagem está grande. Use um logo menor (até ~700 KB).");
      return;
    }
    ...
    <div style={styles.page}>
      <div style={styles.container}>
        ...
        <header style={styles.header}>
          <div style={styles.brandRow}>
            <div style={styles.brandBlock}>
              <div style={styles.appTitle}>StudioFlow</div>
              <div style={styles.appSlogan}>organize, edite e entregue sem perder prazos</div>
            </div>

            <div style={styles.userBrandBlock}>
              <div style={styles.userBrandInner}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    <label style={styles.btnGhostLabelSmall}>
                    ...
                    {brand.logoDataUrl ? (
                      <button style={styles.btnGhostSmall} onClick={clearLogo}>
                        Remover
                      </button>
                    ) : null}
                  </div>
                  ...
                  filtered.map((j) => {
                    ...
                    return (
                      <button
                        ...
                        style={{ ...styles.listItem, ...toneBox, outline: "none" }}
                      >
        ...
        <footer style={styles.footer}>
          StudioFlow • Local-first • Próximo passo: sincronizar em nuvem + lembretes automáticos reais
        </footer>
      </div>
...
const styles = {
  ...
  appTitle: { fontSize: 26, fontWeight: 900, letterSpacing: 0.2 },
  appSlogan: { marginTop: 4, fontSize: 13, color: "rgba(255,255,255,0.70)" },
  ...
  userBrandInner: { display: "flex", gap: 12, alignItems: "center" },
  logoBox: {
    width: 54,
    height: 54,
    borderRadius: 14,
  ...
  logoPlaceholder: { fontSize: 12, fontWeight: 900, color: "rgba(255,255,255,0.55)" },
